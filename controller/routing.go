...
func SetupRouter(signingData *common.TokenSigningData, database repository.Database, router *mux.Router, ctrlOrganization OrganizationController, ctrlGitSource GitSourceController, ctrlWebHook WebHookController, ctrlTrigger TriggersController, ctrlOauth2 Oauth2Controller, ctrlUser UserController) {
	...
	setupStopTriggersConfigEndpoint(apirouter.PathPrefix("/stoptriggers").Subrouter(), ctrlTrigger)
	setupStartTriggersConfigEndpoint(apirouter.PathPrefix("/starttriggers").Subrouter(), ctrlTrigger)
	...
	setupGetAllRunningRuns(apirouter.PathPrefix("/agolarunningruns").Subrouter(), ctrlUser)
...
func setupStopTriggersConfigEndpoint(router *mux.Router, ctrl TriggersController) {
	router.Use(handleLoggedUserWithAdminRoleRoutes)
	router.HandleFunc("", ctrl.StopTriggers).Methods("POST")
}

func setupStartTriggersConfigEndpoint(router *mux.Router, ctrl TriggersController) {
	router.Use(handleLoggedUserWithAdminRoleRoutes)
	router.HandleFunc("", ctrl.StartTriggers).Methods("POST")
}
...
func setupGetAllRunningRuns(router *mux.Router, ctrl UserController) {
	router.Use(handleRestrictedAllRoutes)
	router.HandleFunc("", ctrl.GetAllAgolaRunningRuns).Methods("GET")
}
...
func handleLoggedUserWithAdminRoleRoutes(h http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if !user.IsAdmin {
			log.Println("user", userId, "not admin")
