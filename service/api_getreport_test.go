...
func TestGetReportOnlyownerWhenIsNotOwnerButOk(t *testing.T) {
	ctl := gomock.NewController(t)
	defer ctl.Finish()
	...
	db.EXPECT().GetGitSourceByName(gomock.Eq(user.GitSourceName)).Return(&gitSource, nil)
	db.EXPECT().GetOrganizations().Return(&organizationList, nil)
	giteaApi.EXPECT().IsUserOwner(gomock.Any(), gomock.Any(), gomock.Any()).Return(false, nil)
	...
	client := ts.Client()
	resp, err := client.Get(ts.URL + "/getReport?onlyowner")

	assert.Equal(t, err, nil)
	assert.Equal(t, resp.StatusCode, http.StatusOK, "http StatusCode is not OK")

	// when ?onlyowner is invalid
	client = ts.Client()
	resp, err = client.Get(ts.URL + "/getReport?onlyowner=invalid")
	...
	// when git sour not found
	db.EXPECT().GetUserByUserId(*user.UserID).Return(user, nil)
	db.EXPECT().GetGitSourceByName(gomock.Eq(user.GitSourceName)).Return(nil, nil)
	client = ts.Client()
	resp, err = client.Get(ts.URL + "/getReport?onlyowner")
	assert.Equal(t, err, nil)
	assert.Equal(t, resp.StatusCode, http.StatusInternalServerError, "http StatusCode is not OK")
