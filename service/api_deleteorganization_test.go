...
func TestDeleteOrganizationWhenUserNotFound(t *testing.T) {
	ctl := gomock.NewController(t)
	defer ctl.Finish()
...
func TestDeleteOrganizationWhenUserIsNotOwner(t *testing.T) {
	ctl := gomock.NewController(t)
	defer ctl.Finish()
	...
	db.EXPECT().GetOrganizationByAgolaRef(gomock.Eq(organization.AgolaOrganizationRef)).Return(&organization, nil)
	db.EXPECT().GetGitSourceByName(gomock.Any()).Return(&gitSource, nil)
	giteaApi.EXPECT().IsUserOwner(gomock.Any(), gomock.Any(), organization.GitPath).Return(false, nil)
	...
	data, _ := json.Marshal(dto.DeleteOrganizationResponseDto{ErrorCode: dto.UserNotOwnerError})
	...
	responseDto := dto.DeleteOrganizationResponseDto{ErrorCode: dto.UserNotOwnerError}
...
func TestDeleteOrganizationWhenUserCreatorNotFound(t *testing.T) {
	ctl := gomock.NewController(t)
	defer ctl.Finish()
	...
	assert.Equal(t, err, nil)
	assert.Equal(t, resp.StatusCode, http.StatusInternalServerError, "UserCreator not found")
